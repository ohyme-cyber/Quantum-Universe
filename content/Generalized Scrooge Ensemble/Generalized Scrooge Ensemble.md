...
Example:
1. Global H satisfies all k-th non-resonance conditions but is time-independent, then $E$ is the global conserved quantity. If measurement basis is energy non-revealing, then $\mathcal{E}=\mathcal{E}_{Scrooge}[\rho_{Gibbs}]$. While if the measurement basis is energy revealing, then $\mathcal{E}=\sum_{x_{B}}p(x_{B})\mathcal{E}_{Scrooge}[\rho_{x_{B}}]$, where $\rho$
